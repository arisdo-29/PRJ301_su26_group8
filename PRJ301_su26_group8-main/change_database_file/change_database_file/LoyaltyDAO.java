 ...
 *
 * THAY ĐỔI so với v1:
 *   getActiveRewards(): đổi tên cột valid_from → start_date, valid_to → end_date
 *                       thêm đọc cột description
 *   redeem()          : đổi tên cột valid_from → start_date, valid_to → end_date
 *
 * Không còn bảng promotions/promotion_services.
 * Khuyến mãi của admin là các reward có points_cost = 0.
 * Trang my_points.jsp hiện các reward có points_cost > 0 (customer đổi điểm).
 */
public class LoyaltyDAO {
    ...
    // ── GET ACTIVE REWARDS (chỉ loại customer đổi điểm) ─────────
    /**
     * Trả về danh sách reward có points_cost > 0 mà user đủ điều kiện.
     * Reward có points_cost = 0 là khuyến mãi admin, không hiển thị ở đây.
     *
     * THAY ĐỔI: đổi tên cột valid_from → start_date, valid_to → end_date
     *           thêm đọc cột description
     */
    public List<Reward> getActiveRewards(int userTierId) {
        ...
        try {
            cn = DBUtils.getConnection();
            // THAY ĐỔI: start_date / end_date thay cho valid_from / valid_to
            // Thêm: points_cost > 0 → chỉ lấy reward đổi điểm (không lấy khuyến mãi admin)
            ...
            while (rs.next()) {
                ...
                r.setDescription(rs.getString("description")); // THÊM MỚI
                ...
                r.setStartDate(rs.getDate("start_date"));  // THAY ĐỔI (validFrom → startDate)
                r.setEndDate(rs.getDate("end_date"));      // THAY ĐỔI (validTo   → endDate)
    ...
    // ── GET ACTIVE PROMOTIONS (loại admin cấu hình, points_cost = 0) ──
    /**
     * Dùng để hiển thị các khuyến mãi đang áp dụng cho user.
     * THÊM MỚI: phương thức này không có trong v1 (trước đây là bảng riêng).
     */
    public List<Reward> getActivePromotions(int userTierId) {
        List<Reward> list = new ArrayList<>();
        Connection cn = null;
        try {
            cn = DBUtils.getConnection();
    ...
    public String redeem(int accountId, int rewardId, int userTierId) {
        ...
        try {
            ...
            java.sql.Date startDate = rsR.getDate("start_date");  // THAY ĐỔI: valid_from → start_date
            java.sql.Date endDate   = rsR.getDate("end_date");    // THAY ĐỔI: valid_to   → end_date
            stReward.close();
