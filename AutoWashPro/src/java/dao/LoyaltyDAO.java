    ...
    public String redeem(int accountId, int rewardId, int userTierId) {
        ...
        try {
            ...
            PreparedStatement stReward = cn.prepareStatement(
                "SELECT points_cost, min_tier_id, is_active, valid_to FROM rewards WHERE id=?");
            ...
            if (validTo != null && validTo.before(new java.sql.Date(System.currentTimeMillis()))) {
                cn.rollback(); return "Phần thưởng đã hết hạn.";
            }
            if (!rsR.wasNull() && userTierId < minTierId) {
            ...
            PreparedStatement stRed = cn.prepareStatement(
                "INSERT INTO redemptions (account_id, reward_id, points_spent, status, expires_at) "
              + "VALUES (?, ?, ?, 'PENDING', DATEADD(day, 30, CAST(GETDATE() AS DATE)))");
