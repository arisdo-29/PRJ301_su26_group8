...
<%--
    my_points.jsp - Trang Diem thuong & Doi thuong cua Customer

    DA SUA (theo yeu cau):
    1) Loi phong chu: them Google Fonts "Inter" (truoc day chi khai bao
       font-family: Inter trong CSS nhung khong import nen bi fallback).
    2) BUG QUAN TRONG: truoc day trang nay CHI hien thi "rewards" (phan
       thuong doi diem, points_cost > 0), hoan toan KHONG hien thi
       "promotions" (khuyen mai, points_cost = 0) du servlet ViewPoints
       (sau khi sua) da co lay du lieu promotions va set vao request.
       -> Da them khu vuc moi "🏷️ Khuyến mãi đang diễn ra" ngay duoi
          khu vuc "🎁 Đổi thưởng", hien thi cac promotion dang active.
          Promotion KHONG can nut "Doi ngay" vi no tu dong ap dung,
          khac voi Reward phai dung diem de doi.
--%>
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* MOI THEM: khu vuc Khuyen mai (Promotion) - mau xanh la phan biet voi Reward */
    .promo-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px; margin-bottom: 32px; }
    .promo-card { background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,.06);
                  display: flex; flex-direction: column; gap: 10px; border-left: 4px solid #059669; }
    .promo-name { font-weight: 800; font-size: 15px; color: #1A2B3C; }
    .promo-desc { font-size: 13px; color: #718096; }
    .promo-value { font-size: 20px; font-weight: 900; color: #059669; }
    .promo-type { font-size: 12px; background: #ECFDF5; color: #059669; padding: 3px 10px; border-radius: 50px; font-weight: 700; display: inline-block; width: fit-content; }
    .promo-auto-tag { font-size: 12px; color: #059669; font-weight: 700; margin-top: auto; }
...
<%
    ...
    List<Reward>    promotions = (List<Reward>) request.getAttribute("promotions"); // MỚI THÊM
...
<div class="container">
  ...
  <%-- ═══════════════════════════════════════════════════
       MỚI THÊM: KHU VỰC KHUYẾN MÃI (Promotion, points_cost = 0)
       Khác với Reward, Promotion KHÔNG cần nút "Đổi ngay" vì admin
       cấu hình áp dụng tự động trong khoảng thời gian hiệu lực,
       khách hàng không cần dùng điểm để đổi.
       ═══════════════════════════════════════════════════ --%>
  <% if (promotions != null && !promotions.isEmpty()) { %>
  <div class="section-title">🏷️ Khuyến mãi đang diễn ra</div>
  <div class="promo-grid">
    <% for (Reward p : promotions) { %>
    <div class="promo-card">
      <div class="promo-name"><%= p.getName() %></div>
      <% if (p.getDescription() != null) { %>
        <div class="promo-desc"><%= p.getDescription() %></div>
      <% } %>
      <div class="promo-type"><%= p.getType() %></div>
      <div class="promo-value">
        <% if ("DISC_PERCENT".equals(p.getType())) { %>
          Giảm <%= (int) p.getValue() %>%
        <% } else if ("DISC_VND".equals(p.getType())) { %>
          Giảm <%= String.format("%,.0f", p.getValue()) %>đ
        <% } else if ("FREE_WASH".equals(p.getType())) { %>
          Miễn phí 1 lần rửa
        <% } else { %>
          Ưu đãi đặc biệt
        <% } %>
      </div>
      <div class="promo-auto-tag">✓ Tự động áp dụng khi thanh toán</div>
    </div>
    <% } %>
  </div>
  <% } %>
